	...
	return (
		<form
			...
			<div className="relative w-full md:w-9/12">
				<input
					className={`bg-white/50 rounded border border-ridge py-2 px-4 focus:outline-none w-full text-center md:text-left ${
						status === "error"
							? "border-danger-500"
							: "border-dark-100"
					}`}
				...
				/>
				{status === "error" && (
					<p className="absolute top-[3rem] text-xs font-semibold text-danger-500 text-left">
						{message}
					</p>
				)}
			</div>
